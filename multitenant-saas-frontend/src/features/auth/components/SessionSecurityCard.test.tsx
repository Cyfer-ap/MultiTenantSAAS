...
function LoginDestination() {
    ...
    return (
        <section>
            ...
            <output aria-label="Login tenant ID">
                {String(state.tenantId ?? '')}
            </output>
            <output aria-label="Login email">
                {String(state.email ?? '')}
            </output>
...
describe('SessionSecurityCard', () => {
    ...
    it('revokes all refresh sessions and clears only the tenant session', async () => {
        ...
        expect(screen.getByLabelText(/login tenant id/i))
            .toHaveTextContent('tenant-1')
        expect(screen.getByLabelText(/login email/i))
            .toHaveTextContent('grace@example.com')
        expect(screen.getByLabelText(/logout-all confirmation/i))
    ...
    it('keeps sessions intact when the backend request fails', async () => {
        vi.spyOn(authApi, 'logoutAllDevices')
