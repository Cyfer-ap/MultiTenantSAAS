...
import { useCreateInvitation, useRevokeInvitation } from '../hooks/useTenantInvitations'
...
interface RevokeInvitationDialogProps extends DialogBaseProps {
...
export function CreateInvitationDialog({ open, tenantId, onClose }: DialogBaseProps) {
            ...
            <Dialog fullWidth maxWidth="sm" onClose={closeDialog} open={open}>
                ...
                <DialogContent>
                    <Stack spacing={2} sx={{ marginTop: 1 }}>
                        ...
                        <DialogContentText>
                            Send this one-time link to {mutation.data.email}. It is available only
                            after creation in the current development environment.
                        </DialogContentText>
                        ...
                        {acceptanceLink ? (
                            ...
                            <Alert severity="info">
                                The invitation was created. Delivery is handled by the configured
                                email service.
                            </Alert>
                        )}

                        {copyFeedback && <Alert severity="info">{copyFeedback}</Alert>}
                    </Stack>
                </DialogContent>
                <DialogActions>
                    {acceptanceLink && (
            ...
            >
                ...
                <DialogContent>
                    <DialogContentText sx={{ marginBottom: 2 }}>
                        Invite a person to set their password and join this tenant with the selected
                        role.
                    </DialogContentText>
...
export function RevokeInvitationDialog({
    ...
    const revoke = async (): Promise<void> => {
        try {
            await mutation.mutateAsync(invitation.invitationId)
            onSuccess(`Invitation for ${invitation.email} was revoked.`)
            onClose()
        } catch {
            // The mutation error is rendered in the dialog.
        }
    ...
    return (
        <Dialog onClose={closeDialog} open={open}>
            ...
            <DialogContent>
                <DialogContentText>
                    {invitation.fullName} will no longer be able to use this invitation to join the
                    tenant.
                </DialogContentText>
