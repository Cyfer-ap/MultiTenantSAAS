...
export function getSubscriptionEndingSoonMessage(
    ...
    if (
        ...
        isEndingSoon(access.currentPeriodEnd, now)
    ) {
        return `Your subscription is scheduled to end on ${formatDate(
            access.currentPeriodEnd,
        )}.`
    }

    if (
        access.accessReason === 'TRIAL_ACTIVE' &&
        isEndingSoon(access.trialEndsAt, now)
    ) {
        return `Your trial ends on ${formatDate(
            access.trialEndsAt,
        )}.`
    }

    if (
        access.accessReason === 'PAST_DUE_GRACE' &&
        isEndingSoon(access.currentPeriodEnd, now)
    ) {
        return `Past-due grace access ends on ${formatDate(
            access.currentPeriodEnd,
        )}.`
    }
...
export function SubscriptionEndingSoonAlert() {
    const access =
        useWorkspaceSubscriptionAccessContext()
    const message =
        getSubscriptionEndingSoonMessage(access)
    ...
    return (
        <Alert
            aria-label="Subscription ending soon"
            severity="warning"
            sx={{ marginBottom: 2 }}
        >
            {message}
