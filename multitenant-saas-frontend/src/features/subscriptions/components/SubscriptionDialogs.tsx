...
export function SubscriptionPlanDialog({
    ...
    const [maxStorageMb, setMaxStorageMb] = useState(String(plan?.maxStorageMb ?? ''))
                ...
                >
                    {(validationError || mutation.isError) && (
                        <Alert severity="error">
                            {validationError ??
                                errorMessage(mutation.error, 'The plan could not be saved.')}
                        </Alert>
                    )}
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                        <TextField
                            disabled={!creating || mutation.isPending}
                            fullWidth
                            label="Plan code"
                            onChange={(event) => {
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                        ...
                        <TextField
                            ...
                            onChange={(event) => {
                                setMaxStorageMb(event.target.value)
                            }}
                            type="number"
                            value={maxStorageMb}
                        />
                ...
                >
                    ...
                    <FormControl fullWidth>
                        ...
                        <Select
                            ...
                            labelId="start-status-label"
                            onChange={(event) => {
                                setStatus(event.target.value as 'TRIALING' | 'ACTIVE')
                            }}
                            value={status}
                        >
                    ...
                    <FormControlLabel
                        control={
                            <Checkbox
                                ...
                                onChange={(event) => {
                                    setCancelAtPeriodEnd(event.target.checked)
                                }}
                            />
                        }
                        label="Cancel automatically at period end"
                    />
                </Stack>
            </DialogContent>
            <DialogActions>
                ...
                <Button
                    disabled={mutation.isPending || activePlans.length === 0}
                    form="start-subscription-form"
                    type="submit"
                    variant="contained"
                >
                    Start subscription
    ...
    return (
        <Dialog fullWidth maxWidth="sm" onClose={onClose} open={open}>
            ...
            <DialogContent>
                <Stack
                    component="form"
                    id="change-subscription-plan-form"
                    onSubmit={(event) => {
                        void submit(event)
                    }}
                    spacing={2}
                ...
                >
                    {mutation.isError && (
                        <Alert severity="error">
                            {errorMessage(mutation.error, 'The plan could not be changed.')}
                        </Alert>
                    )}
                    <FormControl fullWidth>
            ...
            <DialogActions>
                ...
                <Button
                    disabled={mutation.isPending}
                    form="change-subscription-plan-form"
                    type="submit"
                    variant="contained"
                >
                    Change plan
...
export function UpdateTenantLifecycleDialog({
    ...
    const [trialEndsAt, setTrialEndsAt] = useState(
        subscription.trialEndsAt ? localDateTime(subscription.trialEndsAt) : '',
    )
    ...
    return (
        <Dialog fullWidth maxWidth="sm" onClose={onClose} open={open}>
            ...
            <DialogContent>
                <Stack
                    component="form"
                    id="update-subscription-lifecycle-form"
                    onSubmit={(event) => {
                        void submit(event)
                    }}
                    spacing={2}
                ...
                >
                    {mutation.isError && (
                        <Alert severity="error">
                            {errorMessage(mutation.error, 'The lifecycle could not be updated.')}
                        </Alert>
                    )}
                    <FormControl fullWidth>
                        <InputLabel id="lifecycle-status-label">Status</InputLabel>
                        <Select
                            label="Status"
                            labelId="lifecycle-status-label"
                            onChange={(event) => {
                                setStatus(event.target.value as TenantSubscriptionStatus)
                            }}
                            value={status}
                        >
                            <MenuItem value="TRIALING">Trialing</MenuItem>
            ...
            <DialogActions>
                ...
                <Button
                    disabled={mutation.isPending}
                    form="update-subscription-lifecycle-form"
                    type="submit"
                    variant="contained"
                >
                    Update lifecycle
