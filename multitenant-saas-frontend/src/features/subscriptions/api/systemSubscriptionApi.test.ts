...
describe('systemSubscriptionApi', () => {
    ...
    it('manages the subscription plan catalog', async () => {
        ...
        const patch = vi
            ...
            .mockResolvedValue(successfulResponse(plan))
        const createInput = {
            code: 'GROWTH',
            name: 'Growth',
            description: null,
        ...
        expect(get).toHaveBeenCalledWith('/api/system/subscription-plans', {
        ...
        expect(patch).toHaveBeenCalledWith('/api/system/subscription-plans/plan-1/status', {
            status: 'INACTIVE',
        })
    })
    ...
    it('manages a tenant subscription through protected system endpoints', async () => {
        const get = vi
            ...
            .mockResolvedValue(successfulResponse(subscription))
        const post = vi
            .spyOn(systemHttpClient, 'post')
            .mockResolvedValue(successfulResponse(subscription))
        const put = vi
            .spyOn(systemHttpClient, 'put')
            .mockResolvedValue(successfulResponse(subscription))
        const patch = vi
            .spyOn(systemHttpClient, 'patch')
            .mockResolvedValue(successfulResponse(subscription))
        ...
        expect(put).toHaveBeenCalledWith(
            '/api/system/tenants/tenant-1/subscription/plan',
            changeInput,
        )
        expect(patch).toHaveBeenCalledWith(
            '/api/system/tenants/tenant-1/subscription/lifecycle',
            lifecycleInput,
        )
    })
})
