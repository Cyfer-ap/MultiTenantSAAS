...
function DashboardContent({ summary }: { summary: SystemDashboardSummary }) {
    return (
        <>
            ...
            >
                <MetricCard
                    detail={summary.activeTenants.toLocaleString() + ' active workspaces'}
                ...
                <MetricCard
                    detail={
                        (summary.inactiveTenants + summary.suspendedTenants).toLocaleString() +
                        ' require attention'
                    }
                ...
                />
                <MetricCard
                    detail={summary.activeUsers.toLocaleString() + ' active accounts'}
                ...
                <MetricCard
                    detail={
                        (summary.inactiveUsers + summary.suspendedUsers).toLocaleString() +
            ...
            >
                <Card variant="outlined">
                    <CardContent>
                        <Typography sx={{ mb: 2, fontWeight: 700 }}>Tenant health</Typography>
                        <Breakdown
                ...
                <Card variant="outlined">
                    <CardContent>
                        <Typography sx={{ mb: 2, fontWeight: 700 }}>User account health</Typography>
                        <Breakdown
...
export function SystemDashboardPage() {
    ...
    return (
        <Box>
            ...
            >
                <Box>
                    ...
                    <Typography color="text.secondary" sx={{ mt: 0.5 }}>
                        Platform-wide tenant and user health.
                    </Typography>
                </Box>
                <Button
            ...
            {query.isLoading && (
                <Box
                    aria-label="Loading global dashboard"
                    role="status"
                    sx={{
                        display: 'grid',
                        gap: 2,
                ...
                >
                    {[0, 1, 2, 3].map((item) => (
                        <Skeleton height={148} key={item} variant="rounded" />
                    ))}
                </Box>
            )}
            {query.isError && (
                ...
                >
                    {query.error instanceof Error
                        ? query.error.message
                        : 'The global dashboard could not be loaded.'}
                </Alert>
            )}
