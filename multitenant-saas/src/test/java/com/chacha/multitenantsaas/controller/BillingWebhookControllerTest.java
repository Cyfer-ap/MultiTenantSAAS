...
class BillingWebhookControllerTest {
    ...
    void verifiesAndPersistsStripeWebhook() {
        ...
        VerifiedBillingEvent event =
                new VerifiedBillingEvent(
                        BillingProviderType.STRIPE, "evt_123", "invoice.paid", "payload");
        BillingWebhookReceipt receipt =
                new BillingWebhookReceipt(
                        BillingProviderType.STRIPE, "evt_123", "invoice.paid", false);
    ...
    void acknowledgesRazorpayDuplicate() {
        ...
        BillingWebhookReceipt receipt =
                new BillingWebhookReceipt(
                        BillingProviderType.RAZORPAY, "event_123", "subscription.activated", true);
        when(razorpayVerifier.verify("payload", "signature", "event_123")).thenReturn(event);
        when(service.ingest(event)).thenReturn(receipt);

        ResponseEntity<ApiResponse<BillingWebhookReceipt>> response =
                controller.receiveRazorpayWebhook("payload", "signature", "event_123");

        assertThat(response.getBody()).isNotNull();
        assertThat(response.getBody().message()).isEqualTo("Billing webhook already received");
