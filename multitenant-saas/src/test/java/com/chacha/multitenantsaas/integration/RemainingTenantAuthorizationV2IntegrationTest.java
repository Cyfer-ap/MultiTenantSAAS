...
class RemainingTenantAuthorizationV2IntegrationTest {
    ...
    private static final String PASSWORD = "RemainingTenant@123";
    ...
    @Test
    void tenantReadAndUpdatePermissionsAreSeparated() throws Exception {

        TestContext context = createContext("tenant-profile");

        AppUser reader = createUser(context.tenant(), "Delegated Tenant Reader");
        ...
        assignRole(context, reader, readerRole);

        String readerToken = login(context.tenant().getId(), reader.getEmail());

        mockMvc.perform(
                        get("/api/tenants/{tenantId}", context.tenant().getId())
                                .header(HttpHeaders.AUTHORIZATION, bearer(readerToken)))
                .andExpect(status().isOk());

        mockMvc.perform(
                        get("/api/tenants/slug/{slug}", context.tenant().getSlug())
                                .header(HttpHeaders.AUTHORIZATION, bearer(readerToken)))
                .andExpect(status().isOk());

        mockMvc.perform(
                        put("/api/tenants/{tenantId}", context.tenant().getId())
                                .header(HttpHeaders.AUTHORIZATION, bearer(readerToken))
                                ...
                                .content(
                                        ...
                                        """
                                                .formatted(context.tenant().getSlug())))
                .andExpect(status().isForbidden());

        AppUser editor = createUser(context.tenant(), "Delegated Tenant Editor");
        ...
        AuthorizationRoleResponse editorRole =
                createRole(
                        ...
                        Set.of(
                                ...
                                PlatformPermissionCodes.TENANT_UPDATE));

        assignRole(context, editor, editorRole);

        String editorToken = login(context.tenant().getId(), editor.getEmail());

        String updatedSlug = "updated-" + uniqueSuffix();
        ...
        mockMvc.perform(
                        put("/api/tenants/{tenantId}", context.tenant().getId())
                                ...
                                .content(
                                        ...
                                        """
                                                .formatted(updatedSlug)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.name").value("Updated Tenant Profile"))
                .andExpect(jsonPath("$.data.slug").value(updatedSlug));
    }

    @Test
    void invitationAcceptanceCreatesGeneratedV2Role() throws Exception {

        TestContext context = createContext("invitation-v2");

        AppUser invitationManager = createUser(context.tenant(), "Delegated Invitation Manager");
        ...
        assignRole(context, invitationManager, invitationRole);

        String managerToken = login(context.tenant().getId(), invitationManager.getEmail());

        String invitedEmail = "invited." + uniqueSuffix() + "@example.test";
        ...
        String invitationToken = invitationBody.path("data").path("devInvitationToken").asText();
        ...
        mockMvc.perform(
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.user.email").value(invitedEmail));

        AppUser invitedUser =
                appUserRepository
                        .findByTenantIdAndEmail(context.tenant().getId(), invitedEmail)
                        .orElseThrow();

        List<AuthorizationUserRoleAssignmentResponse> effectiveAssignments =
                assignmentService.getEffectiveUserAssignments(
                        context.tenant().getId(), invitedUser.getId(), Instant.now());

        List<String> systemRoleCodes =
                effectiveAssignments.stream()
                        .map(AuthorizationUserRoleAssignmentResponse::roleCode)
                        .filter(SystemRoleCodes.ALL::contains)
                        .toList();

        assertEquals(List.of(SystemRoleCodes.MANAGER), systemRoleCodes);
    }

    @Test
    void dashboardRequiresCompleteAggregatePermissionSet() throws Exception {

        TestContext context = createContext("dashboard-v2");

        AppUser dashboardReader = createUser(context.tenant(), "Dashboard Reader");
        ...
        assignRole(context, dashboardReader, dashboardRole);

        String dashboardToken = login(context.tenant().getId(), dashboardReader.getEmail());

        mockMvc.perform(
                        get("/api/tenant/dashboard/summary")
                                .header(HttpHeaders.AUTHORIZATION, bearer(dashboardToken)))
                .andExpect(status().isOk());

        AppUser incompleteReader = createUser(context.tenant(), "Incomplete Dashboard Reader");
        ...
        assignRole(context, incompleteReader, incompleteRole);

        String incompleteToken = login(context.tenant().getId(), incompleteReader.getEmail());

        mockMvc.perform(
                        get("/api/tenant/dashboard/summary")
                                .header(HttpHeaders.AUTHORIZATION, bearer(incompleteToken)))
                .andExpect(status().isForbidden());
    }
