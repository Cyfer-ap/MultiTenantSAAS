...
class ProjectTaskIntegrationTest {
    ...
    @Test
    void tenantAdminCanManageCompleteTaskLifecycle() throws Exception {

        TenantFixture tenant = onboardUniqueTenant("task-lifecycle");

        String adminToken = login(tenant.tenantId(), tenant.adminEmail(), ADMIN_PASSWORD);

        UUID projectId = createProject(tenant.tenantId(), adminToken, "Task Lifecycle Project");
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.data.assigneeUserId").value(tenant.adminUserId().toString()))
        ...
        mockMvc.perform(
                        put(
                                ...
                                .content(
                                        ...
                                        """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.title").value("Implement secure authentication"))
                .andExpect(jsonPath("$.data.priority").value("URGENT"))
                .andExpect(jsonPath("$.data.dueAt").value("2026-08-20T12:00:00Z"));

        updateTaskStatus(tenant.tenantId(), projectId, taskId, adminToken, "IN_PROGRESS")
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.status").value("IN_PROGRESS"));

        updateTaskStatus(tenant.tenantId(), projectId, taskId, adminToken, "COMPLETED")
                .andExpect(status().isOk())
         ...
         */
        updateTaskStatus(tenant.tenantId(), projectId, taskId, adminToken, "BLOCKED")
                .andExpect(status().isOk())
        ...
        mockMvc.perform(
                        patch(
                                ...
                                .content(
                                        ...
                                        """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.assigneeUserId").value(nullValue()))
                .andExpect(jsonPath("$.data.assigneeName").value(nullValue()))
                .andExpect(jsonPath("$.data.assigneeEmail").value(nullValue()));
    ...
    @Test
    void taskListSupportsSearchStatusPriorityAndAssigneeFilters() throws Exception {

        TenantFixture tenant = onboardUniqueTenant("task-filter");

        String adminToken = login(tenant.tenantId(), tenant.adminEmail(), ADMIN_PASSWORD);

        UUID projectId = createProject(tenant.tenantId(), adminToken, "Task Filter Project");

        UUID authenticationTaskId =
                createTask(
                        tenant.tenantId(),
                        projectId,
                        adminToken,
        ...
        mockMvc.perform(
                        get(
                                ...
                                .param("priority", "HIGH")
                                .param("assigneeUserId", tenant.adminUserId().toString())
                                .param("page", "0")
                                .param("size", "10")
                                .param("sortBy", "title")
                                .param("sortDir", "asc"))
                ...
                .andExpect(jsonPath("$.data.content[0].id").value(authenticationTaskId.toString()))
                .andExpect(jsonPath("$.data.content[0].title").value("Implement authentication"))
                .andExpect(jsonPath("$.data.content[0].status").value("IN_PROGRESS"))
                .andExpect(jsonPath("$.data.content[0].priority").value("HIGH"));
    }

    @Test
    void assignedMemberCanUpdateStatusButCannotManageTask() throws Exception {

        TenantFixture tenant = onboardUniqueTenant("task-assigned-member");
        ...
        UUID projectId = createProject(tenant.tenantId(), adminToken, "Assigned Member Project");
        ...
        updateTaskStatus(tenant.tenantId(), projectId, taskId, memberToken, "IN_PROGRESS")
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.status").value("IN_PROGRESS"));

        String taskPath =
                "/api/tenants/" + tenant.tenantId() + "/projects/" + projectId + "/tasks/" + taskId;
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.path").value(taskPath));
    ...
    @Test
    void unassignedProjectMemberCanReadButCannotUpdateTaskStatus() throws Exception {

        TenantFixture tenant = onboardUniqueTenant("task-unassigned-member");
        ...
        UUID projectId = createProject(tenant.tenantId(), adminToken, "Unassigned Member Project");
        ...
        updateTaskStatus(tenant.tenantId(), projectId, taskId, memberToken, "IN_PROGRESS")
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.errorCode").value("ACCESS_DENIED"))
                .andExpect(jsonPath("$.path").value(statusPath));
    }

    @Test
    void projectLeadCanCreateUpdateAssignAndCancelTasks() throws Exception {

        TenantFixture tenant = onboardUniqueTenant("task-project-lead");

        String adminToken = login(tenant.tenantId(), tenant.adminEmail(), ADMIN_PASSWORD);

        UserFixture projectLead =
                inviteAndAcceptUser(
                        tenant.tenantId(),
                        adminToken,
                        "Project Lead User",
        ...
        UUID projectId = createProject(tenant.tenantId(), adminToken, "Project Lead Task Project");

        addProjectMember(
                tenant.tenantId(), projectId, adminToken, projectLead.userId(), "PROJECT_LEAD");

        String projectLeadToken = login(tenant.tenantId(), projectLead.email(), MEMBER_PASSWORD);
        ...
        mockMvc.perform(
                        put(
                                        ...
                                        projectId,
                                        taskId)
                                .header(HttpHeaders.AUTHORIZATION, "Bearer " + projectLeadToken)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                        """
                                        {
                                        ...
                                        """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.title").value("Updated lead task"))
                .andExpect(jsonPath("$.data.priority").value("URGENT"));
    ...
    @Test
    void tenantUserWhoIsNotProjectMemberCannotReadTasks() throws Exception {

        TenantFixture tenant = onboardUniqueTenant("task-non-member");
        ...
        UUID projectId = createProject(tenant.tenantId(), adminToken, "Private Project Tasks");
        ...
        String expectedPath =
                "/api/tenants/" + tenant.tenantId() + "/projects/" + projectId + "/tasks";
    ...
    @Test
    void taskAssigneeMustBeActiveProjectMember() throws Exception {

        TenantFixture tenant = onboardUniqueTenant("task-invalid-assignee");
        ...
        UserFixture tenantUser =
                inviteAndAcceptUser(
                        ...
                        adminToken,
                        "Non Member Assignee",
                        "invalid-assignee." + uniqueSuffix() + "@example.test",
                        "TENANT_USER");

        UUID projectId =
                createProject(tenant.tenantId(), adminToken, "Assignee Validation Project");
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value("Task assignee must be a member " + "of the project"))
                .andExpect(jsonPath("$.path").value(expectedPath));
    }

    @Test
    void crossTenantTaskAccessIsDenied() throws Exception {

        TenantFixture tenantA = onboardUniqueTenant("task-tenant-a");

        TenantFixture tenantB = onboardUniqueTenant("task-tenant-b");
        ...
        UUID tenantBProjectId =
                createProject(tenantB.tenantId(), tenantBToken, "Tenant B Task Project");
        ...
        String expectedPath =
                "/api/tenants/" + tenantB.tenantId() + "/projects/" + tenantBProjectId + "/tasks";
    ...
    @Test
    void cancelledTasksAndArchivedProjectTasksCannotBeModified() throws Exception {

        TenantFixture tenant = onboardUniqueTenant("task-immutable");

        String adminToken = login(tenant.tenantId(), tenant.adminEmail(), ADMIN_PASSWORD);

        UUID projectId = createProject(tenant.tenantId(), adminToken, "Immutable Task Project");
        ...
        mockMvc.perform(
                ...
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.errorCode").value("INVALID_REQUEST"))
                .andExpect(jsonPath("$.message").value("Cancelled task cannot be modified"))
                .andExpect(jsonPath("$.path").value(cancelledTaskPath));

        UUID archivedProjectId =
                createProject(tenant.tenantId(), adminToken, "Archived Task Project");

        UUID archivedProjectTaskId =
                createTask(
                        tenant.tenantId(),
                        archivedProjectId,
                        adminToken,
        ...
        String archivedTaskCollectionPath =
                "/api/tenants/" + tenant.tenantId() + "/projects/" + archivedProjectId + "/tasks";
        ...
        mockMvc.perform(
                ...
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.errorCode").value("INVALID_REQUEST"))
                .andExpect(jsonPath("$.message").value("Archived project tasks cannot be modified"))
                .andExpect(jsonPath("$.path").value(archivedTaskCollectionPath));
        ...
        mockMvc.perform(
                        get(
                                ...
                                .header(HttpHeaders.AUTHORIZATION, "Bearer " + adminToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.id").value(archivedProjectTaskId.toString()));
    }
    ...
    private UUID createTask(
        ...
        MvcResult result =
                mockMvc.perform(
                        ...
                        .andExpect(jsonPath("$.data.priority").value(priority))
                        .andExpect(jsonPath("$.data.status").value("TODO"))
                        .andReturn();
    ...
    private String taskRequest(
            String title, String description, String priority, UUID assigneeUserId) {
        String assigneeJson = assigneeUserId == null ? "null" : "\"" + assigneeUserId + "\"";
    ...
    private UserFixture inviteAndAcceptUser(
            UUID tenantId, String adminToken, String fullName, String email, String tenantRole)
            throws Exception {
        ...
        MvcResult invitationResult =
                mockMvc.perform(
                                post("/api/tenants/{tenantId}/user-invitations", tenantId)
                                        ...
                                        """
                                                        .formatted(fullName, email, tenantRole)))
                        .andExpect(status().isOk())
                        .andReturn();
    ...
    private void addProjectMember(
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.data.projectRole").value(role));
    }
