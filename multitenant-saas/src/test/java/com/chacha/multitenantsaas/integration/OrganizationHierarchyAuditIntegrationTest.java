...
class OrganizationHierarchyAuditIntegrationTest {
    ...
    private static final String PASSWORD = "OrganizationAuditAdmin@123";
    ...
    @Test
    void hierarchyMutationsCreateTenantAuditEntries() throws Exception {

        Tenant tenant = createTenant("organization-audit");

        AppUser admin = createUser(tenant, "Organization Audit Admin");

        String accessToken = login(tenant.getId(), admin.getEmail());
        ...
        mockMvc.perform(
                        patch(
                                ...
                                .content(
                                        ...
                                        """
                                                .formatted(destinationUnitId)))
                .andExpect(status().isOk());

        assertAuditAction(tenant.getId(), admin, accessToken, "ORG_UNIT_CREATED", 2);

        assertAuditAction(tenant.getId(), admin, accessToken, "ORG_UNIT_UPDATED", 1);

        assertAuditAction(tenant.getId(), admin, accessToken, "ORG_UNIT_STATUS_UPDATED", 1);

        assertAuditAction(tenant.getId(), admin, accessToken, "ORG_UNIT_MOVED", 1);

        mockMvc.perform(
                        get("/api/tenants/{tenantId}/audit-logs", tenant.getId())
                                .header(HttpHeaders.AUTHORIZATION, bearer(accessToken))
                                .param("action", "ORG_UNIT_MOVED")
                                .param("page", "0")
                                .param("size", "10"))
                .andExpect(status().isOk())
                .andExpect(
                        jsonPath(
                                ...
                                containsString(targetUnitId.toString())))
                .andExpect(
                        jsonPath(
                                "$.data.content[0].message",
                                containsString(destinationUnitId.toString())));
    }
    ...
    private void assertAuditAction(
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.data.content[0].actorType").value("TENANT_USER"))
                .andExpect(
                        jsonPath("$.data.content[0].actorUserId")
                                .value(expectedActor.getId().toString()))
                .andExpect(
                        jsonPath("$.data.content[0].actorUserEmail")
                                .value(expectedActor.getEmail()))
                .andExpect(
    ...
    private AppUser createUser(Tenant tenant, String fullName) {
        AppUser admin =
                new AppUser(
                        tenant,
                        fullName,
                        "organization.audit." + uniqueSuffix() + "@example.test",
                        passwordEncoder.encode(PASSWORD),
                        UserRole.TENANT_ADMIN);

        AppUser savedAdmin = appUserRepository.saveAndFlush(admin);

        authorizationProvisioningService.synchronizeUserFromLegacyState(
                tenant.getId(), savedAdmin.getId());
