...
class AuthorizationProvisioningReadinessIntegrationTest {
    ...
    @Test
    void backfillRepairsTenantAndIsIdempotent()
            throws Exception {

        TestContext context =
                createContext("readiness");
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath(
                                "$.data.missingSystemRoleCodes"
                        )
                                .isEmpty()
                )
                .andExpect(
                        jsonPath("$.data.usersScanned")
                                .value(4)
                )
                .andExpect(
                        jsonPath("$.data.activeUsers")
                                .value(3)
                )
                .andExpect(
                        jsonPath("$.data.inactiveUsers")
                                .value(1)
                )
                .andExpect(
                        jsonPath("$.data.unresolvedUsers")
                                .value(2)
                );
    ...
    @Test
    void delegatedAuthorizationManagerCanRunBackfill()
            throws Exception {

        TestContext context =
                createContext("delegated-backfill");
        ...
        String delegatedToken =
                login(
                        context.tenant().getId(),
                        delegatedManager.getEmail()
                );

        String readOnlyToken =
                login(
                        context.tenant().getId(),
                        readOnlyUser.getEmail()
                );
    ...
    private void assertSingleSystemRole(
            UUID tenantId,
            UUID userId,
        ...
        assertEquals(
                List.of(expectedRoleCode),
                activeSystemRoleCodes
        );
    }
