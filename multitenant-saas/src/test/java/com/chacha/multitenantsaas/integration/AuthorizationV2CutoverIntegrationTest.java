...
class AuthorizationV2CutoverIntegrationTest {
    ...
    private static final String PASSWORD = "AuthorizationCutover@123";
    ...
    @Test
    void legacyRoleIsDeniedUntilV2BackfillRuns() throws Exception {
         ...
         */
        AppUser unprovisionedLegacyAdministrator = createLegacyAdministrator(context.tenant());
        ...
        String legacyAdministratorToken =
                login(context.tenant().getId(), unprovisionedLegacyAdministrator.getEmail());
        ...
        mockMvc.perform(
                        get("/api/tenants/{tenantId}", context.tenant().getId())
                                .header(
                                        ...
                                        bearer(legacyAdministratorToken)))
        ...
        mockMvc.perform(
                        post(
                                ...
                                .header(
                                        HttpHeaders.AUTHORIZATION,
                                        bearer(provisionedAdministratorToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.provisioning" + ".assignmentsCreated").value(1))
        ...
        mockMvc.perform(
                        get("/api/tenants/{tenantId}", context.tenant().getId())
                                .header(
                                        ...
                                        bearer(legacyAdministratorToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.id").value(context.tenant().getId().toString()));
        ...
        TenantOnboardingResponse onboarding =
                tenantOnboardingService.onboardTenant(
                        new TenantOnboardingRequest(
                                ...
                                "cutover.admin." + suffix + "@example.test",
                                PASSWORD));
    ...
    private AppUser createLegacyAdministrator(Tenant tenant) {
        String suffix = uniqueSuffix();
        ...
        AppUser user =
                new AppUser(
                        ...
                        "legacy.admin." + suffix + "@example.test",
