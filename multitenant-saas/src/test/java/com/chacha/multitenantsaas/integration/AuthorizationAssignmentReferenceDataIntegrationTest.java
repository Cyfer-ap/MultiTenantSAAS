...
class AuthorizationAssignmentReferenceDataIntegrationTest {
    ...
    private static final String PASSWORD = "AuthorizationSelectorAdmin@123";
    ...
    @Test
    void authorizationManagerReceivesOnlyValidSelectorOptions() throws Exception {
        Tenant tenant = createTenant();
        AppUser administrator = createUser(tenant, "Selector Administrator", UserRole.TENANT_ADMIN);
        AppUser member = createUser(tenant, "Grace Selector", UserRole.TENANT_USER);
        AppUser inactiveUser = createUser(tenant, "Inactive Selector", UserRole.TENANT_USER);
        ...
        OrganizationalUnit inactiveUnit =
                new OrganizationalUnit(
                        tenant, null, "Former Division", "FORMER", OrganizationalUnitType.DIVISION);
        inactiveUnit.setStatus(OrganizationalUnitStatus.INACTIVE);
        inactiveUnit = organizationalUnitRepository.saveAndFlush(inactiveUnit);

        Project activeProject =
                projectRepository.saveAndFlush(
                        new Project(tenant, administrator, "Apollo", "Active selector project."));

        Project archivedProject =
                new Project(
                        tenant, administrator, "Archived Initiative", "Must not be selectable.");
        ...
        UserOrganizationAssignment managerAnchor =
                userOrganizationAssignmentRepository.saveAndFlush(
        ...
        String adminToken = login(tenant.getId(), administrator.getEmail());
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.data.users[*].id", hasItem(member.getId().toString())))
                .andExpect(
                        jsonPath(
                                "$.data.users[*].id",
                                not(hasItem(inactiveUser.getId().toString()))))
                .andExpect(
                        jsonPath(
                ...
                .andExpect(
                        jsonPath(
                                "$.data.organizationalUnits[*].id",
                                not(hasItem(inactiveUnit.getId().toString()))))
                .andExpect(
                        jsonPath(
                                "$.data.projects[*].id", hasItem(activeProject.getId().toString())))
                .andExpect(
                        jsonPath(
                                "$.data.projects[*].id",
                                not(hasItem(archivedProject.getId().toString()))))
                .andExpect(
                        jsonPath("$.data.directReportsAnchors[0].id")
                                .value(managerAnchor.getId().toString()))
                .andExpect(
                        jsonPath("$.data.directReportsAnchors[0]" + ".ownerUserId")
                                .value(member.getId().toString()))
                .andExpect(
                        jsonPath("$.data.directReportsAnchors[0].label")
                                .value("Engineering — Engineering Lead"));

        String memberToken = login(tenant.getId(), member.getEmail());
    ...
    private Tenant createTenant() {
        ...
        return tenantRepository.saveAndFlush(
                new Tenant("Authorization Selector Tenant", "authorization-selector-" + suffix));
    }
