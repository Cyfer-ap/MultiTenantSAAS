...
class OrganizationAuthorizationV2IntegrationTest {
    ...
    private static final String PASSWORD = "OrganizationScope@123";
    ...
    @Autowired private AuthorizationUserRoleAssignmentService authorizationAssignmentService;
    ...
    @Test
    void organizationalSubtreeScopeIsIsolated() throws Exception {

        TestContext context = createContext("org-subtree");
        ...
        AuthorizationRoleResponse role =
                createRole(
                        ...
                        Set.of(
                                ...
                                PlatformPermissionCodes.ORGANIZATION_UNIT_MANAGE,
        ...
        String accessToken = login(context.tenant().getId(), context.operator().getEmail());
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.data.id").value(context.backend().getId().toString()));
        ...
        mockMvc.perform(
                        get(unitUrl("/tree"), context.tenant().getId())
                                .header(HttpHeaders.AUTHORIZATION, bearer(accessToken)))
                .andExpect(status().isForbidden());
        ...
        mockMvc.perform(
                        put(
                                ...
                                .content(
                                        ...
                                        """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.name").value("Backend Platform"));
        ...
        mockMvc.perform(
                        post(assignmentUrl(""), context.tenant().getId())
                                .header(HttpHeaders.AUTHORIZATION, bearer(accessToken))
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                        assignmentBody(
        ...
        mockMvc.perform(
                        get(
                                        assignmentUrl("/units/{organizationalUnitId}"),
                                        context.tenant().getId(),
                                        context.backend().getId())
                                .header(HttpHeaders.AUTHORIZATION, bearer(accessToken)))
                .andExpect(status().isOk());
    ...
    @Test
    void exactUnitScopeCannotReadWholeSubtree() throws Exception {

        TestContext context = createContext("org-exact");

        AuthorizationRoleResponse role =
                createRole(
                        context.tenant(),
                        "EXACT_UNIT_READER",
                        Set.of(PlatformPermissionCodes.ORGANIZATION_UNIT_READ));
    ...
    @Test
    void directReportsScopeReadsOnlyDirectReports() throws Exception {

        TestContext context = createContext("org-reports");
        ...
        AuthorizationRoleResponse role =
                createRole(
                        ...
                        Set.of(PlatformPermissionCodes.ORGANIZATION_ASSIGNMENT_READ));
        ...
        mockMvc.perform(
                        get(
                                        assignmentUrl("/{managerAssignmentId}" + "/direct-reports"),
                                        context.tenant().getId(),
                                        managerAssignmentId)
                                .header(HttpHeaders.AUTHORIZATION, bearer(accessToken)))
                .andExpect(status().isOk())
    ...
    private TestContext createContext(String prefix) {
         ...
         */
        AppUser operator = createUser(tenant, prefix + " Operator", UserRole.TENANT_ADMIN);

        AppUser targetUser = createUser(tenant, prefix + " Target", UserRole.TENANT_USER);

        AppUser otherUser = createUser(tenant, prefix + " Other", UserRole.TENANT_USER);
    ...
    private AuthorizationRoleResponse createRole(
        ...
        return authorizationRoleService.createTenantRole(
                tenant.getId(),
                new AuthorizationRoleCreateRequest(roleCode, roleCode, null, permissionIds));
    }
    ...
    private void assignRole(
            ...
            UUID scopeTargetId) {
        authorizationAssignmentService.createAssignment(
                context.tenant().getId(),
                context.administrator().getId(),
    ...
    private AppUser createUser(Tenant tenant, String fullName, UserRole legacyRole) {
        ...
        AppUser user =
                new AppUser(
                        tenant,
                        fullName,
                        "organization." + suffix + "@example.test",
    ...
    private String assignmentBody(UUID userId, UUID unitId, String title) {
        return """
                {
                  "userId": "%s",
                ...
                """
                .formatted(userId, unitId, title);
    }

    private String unitUrl(String suffix) {
        return "/api/tenants/{tenantId}" + "/organization/units" + suffix;
    }

    private String assignmentUrl(String suffix) {
        return "/api/tenants/{tenantId}" + "/organization/assignments" + suffix;
    }
