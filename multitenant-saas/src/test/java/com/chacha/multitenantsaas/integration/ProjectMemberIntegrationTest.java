...
class ProjectMemberIntegrationTest {
    ...
    @Test
    void projectCreatorIsAutomaticallyAssignedAsProjectLead() throws Exception {

        TenantFixture tenant = onboardUniqueTenant("member-auto-lead");

        String adminToken = login(tenant.tenantId(), tenant.adminEmail(), ADMIN_PASSWORD);

        UUID projectId = createProject(tenant.tenantId(), adminToken, "Automatic Lead Project");
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.data.content[0].userId").value(tenant.adminUserId().toString()))
                .andExpect(jsonPath("$.data.content[0].email").value(tenant.adminEmail()))
                .andExpect(jsonPath("$.data.content[0].projectRole").value("PROJECT_LEAD"))
                .andExpect(
                        jsonPath("$.data.content[0].assignedByUserId")
                                .value(tenant.adminUserId().toString()))
                .andExpect(
                        jsonPath("$.data.content[0].assignedByUserEmail")
                                .value(tenant.adminEmail()));
    }

    @Test
    void tenantAdminCanAddSearchPromoteAndRemoveMember() throws Exception {

        TenantFixture tenant = onboardUniqueTenant("member-lifecycle");

        String adminToken = login(tenant.tenantId(), tenant.adminEmail(), ADMIN_PASSWORD);

        UUID projectId =
                createProject(tenant.tenantId(), adminToken, "Membership Lifecycle Project");
        ...
        mockMvc.perform(
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.totalElements").value(1))
                .andExpect(jsonPath("$.data.content[0].userId").value(member.userId().toString()))
                .andExpect(jsonPath("$.data.content[0].email").value(member.email()))
                .andExpect(jsonPath("$.data.content[0].projectRole").value("MEMBER"));
        ...
        mockMvc.perform(
                        patch(
                                ...
                                .content(
                                        ...
                                        """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.projectRole").value("PROJECT_LEAD"));
    ...
    @Test
    void duplicateProjectMembershipIsRejected() throws Exception {

        TenantFixture tenant = onboardUniqueTenant("member-duplicate");

        String adminToken = login(tenant.tenantId(), tenant.adminEmail(), ADMIN_PASSWORD);

        UUID projectId =
                createProject(tenant.tenantId(), adminToken, "Duplicate Membership Project");
        ...
        String requestBody = projectMemberRequest(member.userId(), "MEMBER");

        String expectedPath =
                "/api/tenants/" + tenant.tenantId() + "/projects/" + projectId + "/members";
    ...
    @Test
    void lastProjectLeadCannotBeDemotedOrRemoved() throws Exception {

        TenantFixture tenant = onboardUniqueTenant("member-last-lead");

        String adminToken = login(tenant.tenantId(), tenant.adminEmail(), ADMIN_PASSWORD);

        UUID projectId =
                createProject(tenant.tenantId(), adminToken, "Last Lead Protection Project");
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.message").value("Project must have at least one project lead"))
                .andExpect(jsonPath("$.path").value(memberPath + "/role"));
    ...
    @Test
    void tenantManagerCanManageProjectMembers() throws Exception {

        TenantFixture tenant = onboardUniqueTenant("member-manager");
        ...
        UUID projectId =
                createProject(tenant.tenantId(), managerToken, "Manager Membership Project");
        ...
        mockMvc.perform(
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.content[0].userId").value(manager.userId().toString()))
                .andExpect(jsonPath("$.data.content[0].projectRole").value("PROJECT_LEAD"));

        addProjectMember(tenant.tenantId(), projectId, managerToken, member.userId(), "MEMBER");
    ...
    @Test
    void tenantUserCanReadMembersButCannotManageThem() throws Exception {

        TenantFixture tenant = onboardUniqueTenant("member-read-only");
        ...
        UUID projectId =
                createProject(tenant.tenantId(), adminToken, "Read Only Membership Project");
        ...
        mockMvc.perform(
                        post(
                                ...
                                .header(HttpHeaders.AUTHORIZATION, "Bearer " + userToken)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(projectMemberRequest(tenantUser.userId(), "MEMBER")))
    ...
    @Test
    void crossTenantProjectMembershipAccessIsDenied() throws Exception {

        TenantFixture tenantA = onboardUniqueTenant("member-tenant-a");

        TenantFixture tenantB = onboardUniqueTenant("member-tenant-b");
        ...
        UUID tenantBProjectId =
                createProject(tenantB.tenantId(), tenantBToken, "Tenant B Membership Project");

        String expectedPath =
                "/api/tenants/" + tenantB.tenantId() + "/projects/" + tenantBProjectId + "/members";
    ...
    @Test
    void archivedProjectMembershipCannotBeModified() throws Exception {

        TenantFixture tenant = onboardUniqueTenant("member-archived");
        ...
        UUID projectId =
                createProject(tenant.tenantId(), adminToken, "Archived Membership Project");
        ...
        mockMvc.perform(
                        post(
                                ...
                                .header(HttpHeaders.AUTHORIZATION, "Bearer " + adminToken)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(projectMemberRequest(member.userId(), "MEMBER")))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.errorCode").value("INVALID_REQUEST"))
                .andExpect(
                        jsonPath("$.message")
                                .value("Archived project memberships " + "cannot be modified"))
                .andExpect(jsonPath("$.path").value(expectedPath));
    }
    ...
    private UserFixture inviteAndAcceptUser(
        ...
        MvcResult acceptanceResult =
                mockMvc.perform(
                        ...
                        .andExpect(jsonPath("$.data.user.role").value(tenantRole))
                        .andReturn();
    ...
    private void addProjectMember(
            UUID tenantId, UUID projectId, String accessToken, UUID userId, String projectRole)
            throws Exception {
        ...
        mockMvc.perform(
                        post(
                                ...
                                .header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(projectMemberRequest(userId, projectRole)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.userId").value(userId.toString()))
                .andExpect(jsonPath("$.data.projectRole").value(projectRole));
    }

    private String projectMemberRequest(UUID userId, String projectRole) {
        return """
                {
                  "userId": "%s",
                  "role": "%s"
