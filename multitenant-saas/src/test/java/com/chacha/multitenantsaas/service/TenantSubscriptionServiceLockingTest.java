...
class TenantSubscriptionServiceLockingTest {
    ...
    @BeforeEach
    void setUp() {
        tenantSubscriptionService = new TenantSubscriptionService(
    ...
    void startSubscriptionLocksTenantBeforeDuplicateCheck() {
        ...
        TenantSubscriptionStartRequest request =
                mock(TenantSubscriptionStartRequest.class);

        when(
                tenantLookupService
                        .getActiveByIdForUpdateOrThrow(TENANT_ID)
        ).thenReturn(tenant);
        when(tenantSubscriptionRepository.existsByTenant_Id(TENANT_ID))
                .thenReturn(true);
        ...
        inOrder.verify(tenantLookupService)
                .getActiveByIdForUpdateOrThrow(TENANT_ID);
        inOrder.verify(tenantSubscriptionRepository)
                .existsByTenant_Id(TENANT_ID);
    }
    ...
    void changePlanLoadsSubscriptionWithWriteLock() {
        ...
        when(
                tenantSubscriptionRepository
                        .findByTenantIdWithPlanForUpdate(TENANT_ID)
        ...
        verify(tenantSubscriptionRepository)
                .findByTenantIdWithPlanForUpdate(TENANT_ID);
        verify(tenantSubscriptionRepository, never())
