...
class UserInvitationServiceLockingTest {
    ...
    void createInvitationLocksTenantBeforeReplacementLookup() {
        Tenant tenant = new Tenant(
                "Invitation Lock Tenant",
                "invitation-lock-tenant"
        );
        tenant.setId(TENANT_ID);

        when(tenantRepository.findByIdForUpdate(TENANT_ID))
                .thenReturn(Optional.of(tenant));
        when(appUserRepository.existsByTenantIdAndEmail(
                TENANT_ID,
                EMAIL
        )).thenReturn(false);
        ...
        when(secureTokenService.generateToken())
                .thenReturn(RAW_TOKEN);
        when(secureTokenService.hashToken(RAW_TOKEN))
                .thenReturn(TOKEN_HASH);
        when(userInvitationRepository.save(any(UserInvitation.class)))
                .thenAnswer(invocation ->
                        invocation.getArgument(0, UserInvitation.class)
                );

        invitationService.createInvitation(
                TENANT_ID,
                new UserInvitationCreateRequest(
                        "Invitee",
                        EMAIL,
        ...
        order.verify(tenantRepository)
                .findByIdForUpdate(TENANT_ID);
        order.verify(appUserRepository)
                .existsByTenantIdAndEmail(TENANT_ID, EMAIL);
    ...
    void acceptInvitationUsesWriteLockedTokenLookup() {
        UserInvitation invitation =
                org.mockito.Mockito.mock(UserInvitation.class);
        ...
        verify(userInvitationRepository, never())
    ...
    void revokeInvitationUsesWriteLockedInvitationLookup() {
        ...
        when(currentSystemAdminService.getRequiredActiveSystemAdmin(jwt))
                .thenReturn(systemAdmin);
        when(userInvitationRepository.findByTenantIdAndIdForUpdate(
                TENANT_ID,
                INVITATION_ID
        )).thenReturn(Optional.of(invitation));
        when(invitation.getStatus())
                .thenReturn(UserInvitationStatus.ACCEPTED);
        ...
        verify(userInvitationRepository, never())
                .findByTenant_IdAndId(TENANT_ID, INVITATION_ID);
    }
}
