...
class AuthenticationLoginLockingTest {
    ...
    void tenantLoginUsesWriteLockedUserLookup() {
        ...
        when(appUserRepository.findByTenantIdAndEmailForUpdate(
                TENANT_ID,
                EMAIL
        ...
        verify(appUserRepository)
                .findByTenantIdAndEmailForUpdate(TENANT_ID, EMAIL);
        verify(appUserRepository, never())
                .findByTenantIdAndEmail(TENANT_ID, EMAIL);
    }

    @Test
    void systemAdminLoginUsesWriteLockedLookup() {
        when(systemAdminRepository.findByEmailForUpdate(EMAIL))
                .thenReturn(Optional.empty());

        assertThrows(
                AuthenticationFailedException.class,
                () -> systemAuthService.login(
                        new SystemAdminLoginRequest(
                                EMAIL,
                                PASSWORD
                        )
                )
    ...
    @Test
    void loginTransactionsCommitAuthenticationFailures()
            throws NoSuchMethodException {
        Method tenantLogin = AuthService.class.getMethod(
                "login",
                UUID.class,
                LoginRequest.class
        );
        Method systemAdminLogin = SystemAuthService.class.getMethod(
                "login",
                SystemAdminLoginRequest.class
        );

        Transactional tenantTransactional =
                tenantLogin.getAnnotation(Transactional.class);
