...
class SubscriptionLifecycleGuardServiceTest {
    ...
    @BeforeEach
    void setUp() {
        guardService = new SubscriptionLifecycleGuardService(entitlementService, true);
    }

    @Test
    void activeSubscriptionAllowsBusinessMutation() {
        when(entitlementService.evaluate(TENANT_ID))
                .thenReturn(entitlements(true, SubscriptionAccessReason.ACTIVE));

        assertDoesNotThrow(() -> guardService.requireBusinessMutationAllowed(TENANT_ID));
    ...
    void pastDueGraceAllowsBusinessMutation() {
        when(entitlementService.evaluate(TENANT_ID))
                .thenReturn(entitlements(true, SubscriptionAccessReason.PAST_DUE_GRACE));
    ...
    void blockedSubscriptionProducesReadOnlyRestriction() {
        when(entitlementService.evaluate(TENANT_ID))
                .thenReturn(entitlements(false, SubscriptionAccessReason.CANCELLED));

        SubscriptionRestrictionException exception =
                assertThrows(
                        SubscriptionRestrictionException.class,
                        () -> guardService.requireBusinessMutationAllowed(TENANT_ID));

        assertThat(exception.getRestrictionType()).isEqualTo(RestrictionType.WORKSPACE_READ_ONLY);
        assertThat(exception.getAccessReason()).isEqualTo(SubscriptionAccessReason.CANCELLED);
        assertThat(exception.getResource()).isEqualTo("workspace");
    }

    @Test
    void disabledEnforcementDoesNotEvaluateSubscription() {
        SubscriptionLifecycleGuardService disabled =
                new SubscriptionLifecycleGuardService(entitlementService, false);
    ...
    private TenantSubscriptionEntitlementResponse entitlements(
            boolean mutationsAllowed, SubscriptionAccessReason accessReason) {
        Instant now = Instant.now();

        ResourceEntitlement resource =
                new ResourceEntitlement(0L, null, null, true, false, false, mutationsAllowed);
