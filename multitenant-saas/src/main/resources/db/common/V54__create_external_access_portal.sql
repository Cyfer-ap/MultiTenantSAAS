CREATE TABLE external_access_grants (
    ...
    CONSTRAINT uk_external_access_grant_scope_id UNIQUE (tenant_id, project_id, id),
    CONSTRAINT uk_external_access_grant_invitation_hash UNIQUE (invitation_token_hash),
    ...
    CONSTRAINT ck_external_access_grant_expiry CHECK (expires_at > created_at)
);
...
CREATE INDEX idx_external_access_grants_project_expiry
    ON external_access_grants (tenant_id, project_id, revoked_at, expires_at);
...
CREATE TABLE external_guest_sessions (
    ...
    CONSTRAINT uk_external_guest_session_hash UNIQUE (token_hash),
    CONSTRAINT fk_external_guest_session_grant
        FOREIGN KEY (tenant_id, project_id, grant_id)
            REFERENCES external_access_grants (tenant_id, project_id, id)
            ON DELETE CASCADE,
    CONSTRAINT ck_external_guest_session_expiry CHECK (expires_at > created_at)
);

CREATE INDEX idx_external_guest_sessions_grant_expiry
    ON external_guest_sessions (tenant_id, project_id, grant_id, revoked_at, expires_at);
