...
public class PasswordResetController {
    ...
    @Operation(
            ...
            description =
                    "Returns the same response whether or not the account "
    ...
    public ResponseEntity<ApiResponse<ForgotPasswordResponse>> forgotPassword(
            @PathVariable UUID tenantId, @Valid @RequestBody ForgotPasswordRequest request) {
        ForgotPasswordResponse internalResponse;

        try {
            internalResponse = passwordResetService.forgotPassword(tenantId, request);
        } catch (ResourceNotFoundException | AuthenticationFailedException exception) {
            internalResponse = new ForgotPasswordResponse(FORGOT_PASSWORD_RESPONSE_MESSAGE, null);
        }

        String devResetToken = exposeResetToken ? internalResponse.devResetToken() : null;

        ForgotPasswordResponse response =
                new ForgotPasswordResponse(FORGOT_PASSWORD_RESPONSE_MESSAGE, devResetToken);

        return ResponseEntity.ok(ApiResponse.success(FORGOT_PASSWORD_RESPONSE_MESSAGE, response));
    }

    @Operation(
            summary = "Reset password",
            description =
                    "Uses a valid reset token to set a new password and revoke active refresh tokens.")
