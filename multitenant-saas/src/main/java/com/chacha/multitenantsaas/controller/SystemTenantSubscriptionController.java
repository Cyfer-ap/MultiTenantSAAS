...
@RestController
@RequestMapping("/api/system/tenants/{tenantId}/subscription")
public class SystemTenantSubscriptionController {
    ...
    @PreAuthorize("@systemSecurity.isSystemAdmin()")
    @PostMapping
    public ResponseEntity<ApiResponse<TenantSubscriptionResponse>> startSubscription(
            @PathVariable UUID tenantId,
            @Valid @RequestBody TenantSubscriptionStartRequest request,
            @AuthenticationPrincipal Jwt jwt) {
        return ResponseEntity.ok(
                ApiResponse.success(
                        "Tenant subscription started successfully",
                        subscriptionAdministrationService.startTenantSubscription(
    ...
    public ResponseEntity<ApiResponse<TenantSubscriptionResponse>> changePlan(
            @PathVariable UUID tenantId,
            @Valid @RequestBody TenantSubscriptionPlanChangeRequest request,
            @AuthenticationPrincipal Jwt jwt) {
        return ResponseEntity.ok(
                ApiResponse.success(
                        "Tenant subscription plan changed successfully",
                        subscriptionAdministrationService.changeTenantPlan(
                                tenantId, request, jwt)));
    }

    @PreAuthorize("@systemSecurity.isSystemAdmin()")
    @PatchMapping("/lifecycle")
    public ResponseEntity<ApiResponse<TenantSubscriptionResponse>> updateLifecycle(
            @PathVariable UUID tenantId,
            @Valid @RequestBody TenantSubscriptionLifecycleUpdateRequest request,
            @AuthenticationPrincipal Jwt jwt) {
        return ResponseEntity.ok(
                ApiResponse.success(
                        "Tenant subscription lifecycle updated successfully",
                        subscriptionAdministrationService.updateTenantSubscriptionLifecycle(
                                tenantId, request, jwt)));
    }
}
