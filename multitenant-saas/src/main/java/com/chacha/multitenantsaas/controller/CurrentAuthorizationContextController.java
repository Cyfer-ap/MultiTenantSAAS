...
public class CurrentAuthorizationContextController {
    ...
    private final CurrentAuthorizationContextService currentAuthorizationContextService;

    public CurrentAuthorizationContextController(
            CurrentAuthorizationContextService currentAuthorizationContextService) {
        this.currentAuthorizationContextService = currentAuthorizationContextService;
    }

    @PreAuthorize("@authorizationSecurity" + ".isCurrentTenant(#tenantId)")
    @GetMapping("/me")
    public ResponseEntity<ApiResponse<CurrentAuthorizationContextResponse>>
        ...
        CurrentAuthorizationContextResponse response =
                currentAuthorizationContextService.getCurrentAuthorizationContext(tenantId, jwt);

        return ResponseEntity.ok(
                ApiResponse.success(
                        "Current authorization context " + "fetched successfully", response));
    }
}
