...
public class ApprovalRequest {
    ...
    @Column(name = "workflow_node_key", nullable = false, length = 64) private String workflowNodeKey;
    @Column(name = "task_id", nullable = false) private UUID taskId;
    @Column(name = "actor_user_id", nullable = false) private UUID actorUserId;
    @Enumerated(EnumType.STRING) @Column(nullable = false, length = 20) private ApprovalRequestStatus status;
    @Column(name = "current_stage_index", nullable = false) private int currentStageIndex;
    @Column(name = "approved_next_node_key", length = 64) private String approvedNextNodeKey;
    @Column(name = "rejected_next_node_key", length = 64) private String rejectedNextNodeKey;
    ...
    public void advance() { currentStageIndex++; }
    public void approve() { status = ApprovalRequestStatus.APPROVED; completedAt = Instant.now(); }
    public void reject() { status = ApprovalRequestStatus.REJECTED; completedAt = Instant.now(); }
    ...
    public String getWorkflowNodeKey() { return workflowNodeKey; }
    ...
    public String getApprovedNextNodeKey() { return approvedNextNodeKey; }
    public String getRejectedNextNodeKey() { return rejectedNextNodeKey; }
