...
public class SubscriptionAdministrationService {
    ...
    @Transactional
    public SubscriptionPlanResponse createPlan(SubscriptionPlanCreateRequest request, Jwt jwt) {
        ...
        platformAuditLogService.recordSuccess(
                ...
                "Subscription plan created: " + created.code());
    ...
    public SubscriptionPlanResponse updatePlan(
        ...
        platformAuditLogService.recordSuccess(
                ...
                "Subscription plan updated: " + updated.code());
    ...
    public TenantSubscriptionResponse getTenantSubscription(UUID tenantId) {
        return tenantSubscriptionService.getSubscription(tenantId);
    }

    @Transactional
    public TenantSubscriptionResponse startTenantSubscription(
            UUID tenantId, TenantSubscriptionStartRequest request, Jwt jwt) {
        SystemAdmin actor = currentSystemAdminService.getRequiredActiveSystemAdmin(jwt);

        Tenant tenant = tenantLookupService.getActiveByIdOrThrow(tenantId);

        TenantSubscriptionResponse created =
                tenantSubscriptionService.startSubscription(tenantId, request);

        platformAuditLogService.recordSuccess(
                actor,
                null,
                PlatformAuditAction.TENANT_SUBSCRIPTION_STARTED,
        ...
        auditLogService.recordSystemAdminSuccess(
                ...
                "Subscription started on plan " + created.plan().code());
    ...
    public TenantSubscriptionResponse changeTenantPlan(
            UUID tenantId, TenantSubscriptionPlanChangeRequest request, Jwt jwt) {
        SystemAdmin actor = currentSystemAdminService.getRequiredActiveSystemAdmin(jwt);

        Tenant tenant = tenantLookupService.getByIdOrThrow(tenantId);

        TenantSubscriptionResponse before = tenantSubscriptionService.getSubscription(tenantId);

        TenantSubscriptionResponse updated =
                tenantSubscriptionService.changePlan(tenantId, request);
        ...
        platformAuditLogService.recordSuccess(
                actor,
                null,
                PlatformAuditAction.TENANT_SUBSCRIPTION_PLAN_CHANGED,
                tenant.getSlug() + ": " + message);

        auditLogService.recordSystemAdminSuccess(
                tenant, actor, null, AuditAction.TENANT_SUBSCRIPTION_PLAN_CHANGED, message);

        return updated;
    }

    @Transactional
    public TenantSubscriptionResponse updateTenantSubscriptionLifecycle(
            UUID tenantId, TenantSubscriptionLifecycleUpdateRequest request, Jwt jwt) {
        SystemAdmin actor = currentSystemAdminService.getRequiredActiveSystemAdmin(jwt);
        ...
        TenantSubscriptionResponse updated =
                tenantSubscriptionService.updateLifecycle(tenantId, request);
        ...
        platformAuditLogService.recordSuccess(
                actor,
                null,
                PlatformAuditAction.TENANT_SUBSCRIPTION_LIFECYCLE_UPDATED,
                tenant.getSlug() + ": " + message);

        auditLogService.recordSystemAdminSuccess(
                tenant, actor, null, AuditAction.TENANT_SUBSCRIPTION_LIFECYCLE_UPDATED, message);
