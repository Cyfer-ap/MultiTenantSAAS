...
public class UserInvitationService {
    ...
    public UserInvitationResponse createInvitation(
            UUID tenantId, UserInvitationCreateRequest request, Jwt jwt) {
        Tenant tenant =
                tenantRepository
        ...
        if (currentSystemAdminService.isSystemAdminToken(jwt)) {
            invitedBySystemAdmin = currentSystemAdminService.getRequiredActiveSystemAdmin(jwt);
        } else {
            invitedByUser = currentActorService.getRequiredActiveActor(tenantId, jwt);
        }
    ...
    @Transactional
    public UserInvitationAcceptResponse acceptInvitation(UserInvitationAcceptRequest request) {
        if (!request.newPassword().equals(request.confirmPassword())) {
            throw new IllegalArgumentException("New password and confirm password do not match");
        }

        String tokenHash = secureTokenService.hashToken(request.invitationToken());
        ...
        if (appUserRepository.existsByTenantIdAndEmail(tenant.getId(), invitation.getEmail())) {
            throw new DuplicateResourceException("User already exists for this tenant");
        }

        subscriptionQuotaGuardService.requireUserSlot(tenant.getId());
    ...
    public PageResponse<UserInvitationDetailsResponse> getInvitationsByTenant(
        ...
        Page<UserInvitation> invitations =
                userInvitationRepository.findTenantInvitations(
                        tenantId, status, role, normalizeSearch(search), pageable);

        return new PageResponse<>(
                invitations.getContent().stream().map(this::mapToDetailsResponse).toList(),
    ...
    @Transactional(readOnly = true)
    public UserInvitationDetailsResponse getInvitationById(UUID tenantId, UUID invitationId) {
        UserInvitation invitation = getInvitationOrThrow(tenantId, invitationId);
    ...
    public UserInvitationDetailsResponse revokeInvitation(
            UUID tenantId, UUID invitationId, Jwt jwt) {
        validateManagementActor(tenantId, jwt);

        UserInvitation invitation = getInvitationForUpdateOrThrow(tenantId, invitationId);
        ...
        UserInvitation updatedInvitation = userInvitationRepository.save(invitation);

        return mapToDetailsResponse(updatedInvitation);
    }

    private void revokeExistingPendingInvitations(UUID tenantId, String email) {
        List<UserInvitation> pendingInvitations =
                userInvitationRepository.findByTenantIdAndEmailAndStatusForUpdate(
                        tenantId, email, UserInvitationStatus.PENDING);
    ...
    private void recordUserCreationAudit(UserInvitation invitation, AppUser savedUser) {
        if (invitation.getInvitedBySystemAdmin() != null) {
            auditLogService.recordSystemAdminSuccess(
                    ...
                    "User created after accepting invitation: " + savedUser.getEmail());
    ...
    private UserInvitation getInvitationOrThrow(UUID tenantId, UUID invitationId) {
        return userInvitationRepository
                .findByTenant_IdAndId(tenantId, invitationId)
                .orElseThrow(
                        () ->
    ...
    private UserInvitation getInvitationForUpdateOrThrow(UUID tenantId, UUID invitationId) {
        return userInvitationRepository
                .findByTenantIdAndIdForUpdate(tenantId, invitationId)
                .orElseThrow(
                        () ->
                                new ResourceNotFoundException(
                                        "Invitation not found with id: "
    ...
    private UserInvitationDetailsResponse mapToDetailsResponse(UserInvitation invitation) {
        AppUser invitedByUser = invitation.getInvitedByUser();
        SystemAdmin invitedBySystemAdmin = invitation.getInvitedBySystemAdmin();

        boolean expired =
                invitation.getStatus() == UserInvitationStatus.PENDING && invitation.isExpired();
        ...
        return new UserInvitationDetailsResponse(
                ...
                invitedBySystemAdmin != null ? invitedBySystemAdmin.getId() : null,
                invitedBySystemAdmin != null ? invitedBySystemAdmin.getEmail() : null);
    }
