...
public class AuthorizationAssignmentReferenceDataService {
    ...
    public AuthorizationAssignmentReferenceDataResponse
    getReferenceData(UUID tenantId) {
        tenantLookupService.getActiveByIdOrThrow(tenantId);

        List<UserOption> users =
                appUserRepository.findByTenantId(tenantId)
                        .stream()
                        .filter(
                                user -> user.getStatus()
                                        == UserStatus.ACTIVE
    ...
    private ScopeTargetOption mapOrganizationalUnit(
            OrganizationalUnit unit
    ) {
        String description =
                unit.getType().name().replace('_', ' ');
    ...
    private ScopeTargetOption mapDirectReportsAnchor(
            UserOrganizationAssignment assignment
    ) {
        OrganizationalUnit unit =
                assignment.getOrganizationalUnit();

        String label = unit.getName();

        if (assignment.getPositionTitle() != null
                && !assignment.getPositionTitle().isBlank()) {
            label += " — "
                    + assignment.getPositionTitle();
        }

        String description = assignment.isPrimaryAssignment()
