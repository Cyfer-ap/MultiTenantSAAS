...
public class ProjectTaskService {
    ...
    public ProjectTaskResponse createTask(
            UUID tenantId, UUID projectId, ProjectTaskCreateRequest request, Jwt jwt) {
        Project project = getProjectOrThrow(tenantId, projectId);

        ensureProjectCanBeModified(project);

        AppUser creator = currentActorService.getRequiredActiveActor(tenantId, jwt);

        AppUser assignee = resolveAssignee(tenantId, projectId, request.assigneeUserId());
    ...
    public PageResponse<ProjectTaskResponse> getTasks(
        ...
        return new PageResponse<>(
                tasks.getContent().stream().map(this::mapToResponse).toList(),
                tasks.getNumber(),
                tasks.getSize(),
    ...
    @Transactional(readOnly = true)
    public ProjectTaskResponse getTask(UUID tenantId, UUID projectId, UUID taskId) {
    ...
    public ProjectTaskResponse updateTask(
            UUID tenantId, UUID projectId, UUID taskId, ProjectTaskUpdateRequest request, Jwt jwt) {
        Project project = getProjectOrThrow(tenantId, projectId);
    ...
    public ProjectTaskResponse updateTaskStatus(
        ...
        ProjectTaskStatus previousStatus = task.getStatus();

        if (request.status() == ProjectTaskStatus.CANCELLED) {
            throw new IllegalArgumentException("Use the cancel endpoint to cancel a task");
        }

        task.setStatus(request.status());

        if (request.status() == ProjectTaskStatus.COMPLETED) {
    ...
    public ProjectTaskResponse updateAssignee(
        ...
        AppUser previousAssignee = task.getAssigneeUser();

        AppUser newAssignee = resolveAssignee(tenantId, projectId, request.assigneeUserId());
    ...
    @Transactional
    public ProjectTaskResponse cancelTask(UUID tenantId, UUID projectId, UUID taskId, Jwt jwt) {
        Project project = getProjectOrThrow(tenantId, projectId);
        ...
        auditLogService.recordSuccess(
                ...
                "Task cancelled: " + taskId + " - " + task.getTitle());
    ...
    private AppUser resolveAssignee(UUID tenantId, UUID projectId, UUID assigneeUserId) {
        ...
        AppUser assignee =
                appUserRepository
                        .findByTenantIdAndId(tenantId, assigneeUserId)
                        .orElseThrow(
                                () ->
                                        new ResourceNotFoundException(
                                                "Assignee not found with id: " + assigneeUserId));

        if (assignee.getStatus() != UserStatus.ACTIVE) {
            throw new IllegalArgumentException("Only active users can be assigned tasks");
        }

        boolean projectMember =
                projectMemberRepository.existsByProject_Tenant_IdAndProject_IdAndUser_Id(
                        tenantId, projectId, assigneeUserId);

        if (!projectMember) {
            throw new IllegalArgumentException("Task assignee must be a member of the project");
        }
    ...
    private void ensureProjectCanBeModified(Project project) {
        if (project.getStatus() == ProjectStatus.ARCHIVED) {
            throw new IllegalArgumentException("Archived project tasks cannot be modified");
        }
    }

    private void ensureTaskCanBeModified(ProjectTask task) {
        if (task.getStatus() == ProjectTaskStatus.CANCELLED) {
            throw new IllegalArgumentException("Cancelled task cannot be modified");
        }
    }
