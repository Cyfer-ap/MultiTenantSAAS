...
public class OrganizationAssignmentService {
    ...
    createAssignment(
        ...
        if (request == null) {
            throw new IllegalArgumentException(
                    "Organization assignment request "
                            + "is required."
            );
        }
        ...
        UserOrganizationAssignment savedAssignment =
                assignmentRepository
    ...
    @Transactional(readOnly = true)
    public List<OrganizationAssignmentResponse>
    getUserAssignments(
            UUID tenantId,
            UUID userId
    ) {
    ...
    validatePrimaryAssignmentAvailability(
        ...
        long overlappingAssignmentCount =
                assignmentRepository
                        .countOverlappingActivePrimaryAssignments(
                                tenantId,
                                userId,
                                OrganizationAssignmentStatus.ACTIVE,
                                validFrom,
    ...
    resolveReportingAssignment(
        ...
        if (reportsToAssignment.getStatus()
                != OrganizationAssignmentStatus.ACTIVE) {
            throw new IllegalArgumentException(
                    "Reporting assignment must be active."
            );
        }

        if (reportsToAssignment
                .getUser()
                .getId()
                .equals(assignedUser.getId())) {
            throw new IllegalArgumentException(
                    "A user cannot report to their own "
    ...
    private void validateValidityRange(
        ...
        if (validUntil != null
            ...
            throw new IllegalArgumentException(
                    "Assignment valid-until time "
                            + "must be after valid-from time."
            );
        }
    }

    private String normalizePositionTitle(
            String positionTitle
    ) {
        ...
        if (normalizedTitle.length() > 150) {
            throw new IllegalArgumentException(
                    "Position title must not exceed "
                            + "150 characters."
            );
        }
    ...
    getRequiredActiveUnit(
        ...
        if (unit.getStatus()
                != OrganizationalUnitStatus.ACTIVE) {
            throw new IllegalArgumentException(
                    "Organizational unit must be active."
            );
        }
    ...
    private OrganizationalUnit getUnitOrThrow(
            UUID tenantId,
            UUID organizationalUnitId
    ) {
    ...
    mapToResponse(
        ...
        UserOrganizationAssignment
                managerAssignment =
                assignment.getReportsToAssignment();
